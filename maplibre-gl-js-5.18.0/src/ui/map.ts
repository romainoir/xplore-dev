...
export class Map extends Camera {
    ...
    */
     ...
     * is not active (Passive Terrain Fallback).
    ...
    queryTerrainElevation(lngLatLike: LngLatLike): number | null {
        // 1. If 3D terrain is active, use it directly (mirrors Camera parent logic)
        if (this.terrain) {
            return this.terrain.getElevationForLngLat(LngLat.convert(lngLatLike), this.transform);
        }

        // 2. Fallback for 2D mode: Directly sample DEM tile data from raster-dem sources.
        //    We avoid creating a temporary Terrain instance because its constructor mutates
        //    the underlying TileManager (usedForTerrain, tileSize).
        if (this.style && this.style._loaded) {
            const lngLat = LngLat.convert(lngLatLike);
            for (const id in this.style.tileManagers) {
                const tm = this.style.tileManagers[id];
                if (!tm || tm.getSource().type !== 'raster-dem') continue;

                // Gather all loaded tiles from in-view tiles
                const allTiles = (tm as any)._inViewTiles?.getAllTiles?.() || [];
                ...
                for (const tile of allTiles) {
                    ...
                    // Check if this tile covers the query point at its zoom level
                    const tz = canonical.z;
                    const tc = 1 << tz;
                    const latRad = lngLat.lat * Math.PI / 180;
                    const tx = Math.floor(((lngLat.lng + 180) / 360) * tc);
                    const ty = Math.floor((1 - Math.log(Math.tan(latRad) + 1 / Math.cos(latRad)) / Math.PI) / 2 * tc);

                    if (canonical.x !== tx || canonical.y !== ty) continue;

                    // Found a matching tile with DEM data — bilinear sample
                    const dem = tile.dem;
                    const dim = dem.dim;
                    // Position within tile [0, 1]
                    const fracX = ((lngLat.lng + 180) / 360) * tc - tx;
                    const fracY = (1 - Math.log(Math.tan(latRad) + 1 / Math.cos(latRad)) / Math.PI) / 2 * tc - ty;
                    ...
                    const elevation =
                        dem.get(cx, cy) * (1 - fx) * (1 - fy) +
                        dem.get(cx + 1, cy) * fx * (1 - fy) +
                        dem.get(cx, cy + 1) * (1 - fx) * fy +
                        dem.get(cx + 1, cy + 1) * fx * fy;
