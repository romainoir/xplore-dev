import { haversineDistanceKm } from './geojson-pathfinding.js';

const DEFAULT_SERVICE_URL = 'https://api.maplibre.org/directions/v5';
...
export class MaplibreDirectionsRouter {
  constructor(options = {}) {
    const {
    ...
    this.apiKeyParam = typeof apiKeyParam === 'string' && apiKeyParam.trim().length
      ? apiKeyParam.trim()
      : null;
  ...
  buildRouteRequest(coords, mode) {
    ...
    const coordinatePath = coords.map((coord) => `${coord[0]},${coord[1]}`).join(';');
    const url = new URL(`${this.serviceUrl}/${profile}/${coordinatePath}`);
    ...
    if (this.apiKey && this.apiKeyParam) {
      url.searchParams.set(this.apiKeyParam, this.apiKey);
    }

    if (this.requestParameters && typeof this.requestParameters === 'object') {
      Object.entries(this.requestParameters).forEach(([key, value]) => {
    ...
    const options = this.mergeFetchOptions({ method: 'GET' });
  ...
  extractRouteFeature(payload) {
    ...
    if (Array.isArray(payload.routes) && payload.routes.length) {
      ...
      const segments = legs.map((leg, index) => ({
        distance: Number.isFinite(leg?.distance) ? leg.distance : null,
        duration: Number.isFinite(leg?.duration) ? leg.duration : null,
        ascent: Number.isFinite(leg?.ascent) ? leg.ascent : null,
        descent: Number.isFinite(leg?.descent) ? leg.descent : null,
        start_index: index,
        end_index: index + 1
      }));
      ...
      const summary = {
        ...
        ascent: Number.isFinite(route.ascent) ? route.ascent : null,
        descent: Number.isFinite(route.descent) ? route.descent : null
      };
  ...
  async requestRoute(coords, mode) {
    ...
    try {
      ...
      const error = new Error('MapLibre Directions request failed due to a network error');
      error.cause = networkError;
      throw error;
    }
    if (!response.ok) {
      throw new Error(`MapLibre Directions request failed (${response.status})`);
    }

    const payload = await response.json();
    const feature = this.extractRouteFeature(payload);
    if (!feature || !feature.geometry || !Array.isArray(feature.geometry.coordinates)) {
      throw new Error('MapLibre Directions response did not include a valid route geometry');
    }
  ...
  async getRoute(waypoints, { mode, preservedSegments } = {}) {
    ...
    try {
      ...
      console.warn('MapLibre Directions routing failed. Falling back to the offline router.', error);
      try {
        ...
        if (fallbackRoute && typeof fallbackRoute === 'object') {
          ...
          warnings.push('MapLibre Directions request failed; offline routing was used instead.');
          fallbackRoute.properties = {
            ...properties,
            warnings,
        ...
        const combined = new Error('MapLibre Directions routing failed and offline fallback was unsuccessful');
...
export default MaplibreDirectionsRouter;
