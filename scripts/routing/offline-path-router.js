...
export class OfflineRouter {
  ...
  async getRoute(waypoints, { mode, preservedSegments } = {}) {
    ...
    if (!cacheKey) {
      ...
      routeTimings.pathfinding = performance.now() - pathStart;
      routeTimings.total = performance.now() - routeStartTime;
      console.log('%c[OfflineRouter]', 'color: #2196F3; font-weight: bold', {
        'ensureReady': `${routeTimings.ensureReady.toFixed(1)}ms`,
        'pathfinding': `${routeTimings.pathfinding.toFixed(1)}ms`,
        'total': `${routeTimings.total.toFixed(1)}ms`,
        'segments': routeTimings.segmentCount,
        'preserved': routeTimings.preservedCount,
        'cacheHit': false
      });
      return result;
    }
    ...
    if (this.routeCache.has(cacheKey)) {
      routeTimings.cacheHit = true;
      routeTimings.total = performance.now() - routeStartTime;
      console.log('%c[OfflineRouter]', 'color: #FF9800; font-weight: bold', {
        'cacheHit': true,
        'total': `${routeTimings.total.toFixed(1)}ms`
      });
