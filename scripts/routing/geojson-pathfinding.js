 ...
 * Provides O(log n) insert and extractMin instead of O(n) linear scan.
 */
class BinaryMinHeap {
  constructor() {
    this.heap = [];
    this.indices = new Map(); // key -> heap index for O(1) lookup
  }
  ...
  extractMin() {
    ...
    this.indices.set(this.heap[0].key, 0);
    this._bubbleDown(0);
...
export class GeoJsonPathFinder {
  ...
  findNearestNode(coord) {
    ...
    // Use spatial buckets for O(bucket size) instead of O(n) lookup
    const candidates = this._collectNearbyNodes(coord);
    let best = null;
    ...
    // Fallback to full scan if buckets returned nothing useful
    if (!best && candidates !== this.nodeList) {
  ...
  findNearestPoint(coord) {
    ...
    // Use spatial buckets for initial node search - O(bucket size) instead of O(n)
    const nearbyNodes = this._collectNearbyNodes(coord);
    ...
    // Only check edges from nearby nodes (spatial optimization)
    // This reduces O(E) to O(nearby edges)
    const nearbyNodesForEdges = this._collectNearbyNodes(coord);
    nearbyNodesForEdges.forEach((node) => {
  ...
  buildPath(startKey, endKey, mode) {
    ...
    // A* algorithm with haversine heuristic (faster than Dijkstra for point-to-point)
    // f(n) = g(n) + h(n) where g is actual distance, h is estimated remaining distance
    const endNode = this.nodes.get(endKey);
    const endCoord = endNode?.coord;

    // Heuristic function with caching: straight-line distance to target (admissible heuristic)
    const heuristicCache = new Map();
    const heuristic = (nodeKey) => {
      if (!endCoord) return 0;
      if (heuristicCache.has(nodeKey)) {
        return heuristicCache.get(nodeKey);
      }
      const node = this.nodes.get(nodeKey);
      if (!node?.coord) return 0;
      const h = haversineDistanceKm(node.coord, endCoord);
      heuristicCache.set(nodeKey, h);
      return h;
    };

    const gScore = new Map(); // Actual distance from start
    const fScore = new Map(); // Estimated total distance (g + h)
    ...
    queue.insert(startKey, fScore.get(startKey));

    while (!queue.isEmpty()) {
      const minEntry = queue.extractMin();
      if (!minEntry) {
        break;
      }

      const currentKey = minEntry.key;
      ...
      node.edges.forEach((edge) => {
        ...
        const currentBestG = gScore.get(edge.key);

        if (currentBestG === undefined || tentativeG < currentBestG) {
          gScore.set(edge.key, tentativeG);
          const h = heuristic(edge.key);
          const f = tentativeG + h;
          fScore.set(edge.key, f);
          previous.set(edge.key, currentKey);
          queue.decreaseKey(edge.key, f);
        }
      });
    }
