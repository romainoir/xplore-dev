 ...
 * Based on https://github.com/birkskyum/maplibre-gl-stars
 *
...
function createStarsLayer(options = {}) {
    ...
    return {
        ...
        onAdd: function (map, gl) {
            ...
            // Fragment shader with dynamic density
            ...
            if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) {
                console.error('Stars vertex shader compilation error:', gl.getShaderInfoLog(vertexShader));
            }
            ...
            if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
                console.error('Stars fragment shader compilation error:', gl.getShaderInfoLog(fragmentShader));
            }
            ...
            if (!gl.getProgramParameter(this.program, gl.LINK_STATUS)) {
                console.error('Stars program linking error:', gl.getProgramInfoLog(this.program));
            }
        ...
        render: function (gl, matrix) {
            ...
            if (projectionData.projectionTransition === 0) {
                return; // Don't render stars in Mercator mode
            }

            gl.useProgram(this.program);

            const vec = new Float64Array([0, 0, 0, 1]);
            const mat4Lib = window.mat4;
            const vec4Lib = window.vec4;

            if (mat4Lib && vec4Lib) {
                vec4Lib.transformMat4(vec, vec, transform.modelViewProjectionMatrix);
                vec[0] /= vec[3];
                vec[1] /= vec[3];
                vec[2] /= vec[3];
                vec[3] = 1;
                vec4Lib.transformMat4(vec, vec, transform.inverseProjectionMatrix);
                vec[0] /= vec[3];
                vec[1] /= vec[3];
                vec[2] /= vec[3];
            }
            ...
            // Render at far plane (behind everything)
