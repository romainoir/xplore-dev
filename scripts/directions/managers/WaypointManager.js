...
export class WaypointManager {
     ...
     * @param {number} index - Waypoint index to drag
     * @param {Object} options - Drag options
     * @param {Array} options.prevNeighbor - Previous neighbor coordinates
     * @param {Array} options.nextNeighbor - Next neighbor coordinates
     * @param {string} options.segmentColor - Segment color for preview
     * @param {string} options.segmentMode - Routing mode for the segment
     * @param {boolean} options.isViaInsert - Whether this is a via-point insertion drag
     */
    startDrag(index, options = {}) {
        // Only validate that index is a valid integer (bounds checking done by caller)
        ...
        this._dragPrevNeighbor = options.prevNeighbor ?? this.waypoints[index - 1]?.slice(0, 2) ?? null;
        this._dragNextNeighbor = options.nextNeighbor ?? this.waypoints[index + 1]?.slice(0, 2) ?? null;
        this._dragSegmentColor = options.segmentColor ?? null;
        this._dragSegmentMode = options.segmentMode ?? null;
        this._viaInsertedByDrag = options.isViaInsert ?? false;

        this.callbacks.onDragStart(index, {
            prevNeighbor: this._dragPrevNeighbor,
            nextNeighbor: this._dragNextNeighbor,
            color: this._dragSegmentColor,
            mode: this._dragSegmentMode
        });
    ...
    /**
     * Update the position of the dragged waypoint
     * @param {Array} coords - New coordinates [lng, lat]
     */
    updateDragPosition(coords) {
        ...
        this.waypoints[this.draggedWaypointIndex] = coords.slice();
        this.callbacks.onDragMove(this.draggedWaypointIndex, coords, {
            prevNeighbor: this._dragPrevNeighbor,
            nextNeighbor: this._dragNextNeighbor,
            color: this._dragSegmentColor
        });
    ...
    /**
     * Finish dragging
     * @returns {Object} Info about the completed drag
     */
    finishDrag() {
        const result = {
            wasDragging: this.isDragging,
            waypointIndex: this.draggedWaypointIndex,
            wasViaInsert: this._viaInsertedByDrag,
            segmentMode: this._dragSegmentMode,
            segmentColor: this._dragSegmentColor
        };

        const prevIsDragging = this.isDragging;
        const prevIndex = this.draggedWaypointIndex;
        ...
        if (prevIsDragging) {
            this.callbacks.onDragEnd(prevIndex, result);
        }
    ...
    /**
     * Cancel dragging without triggering callbacks
     */
    cancelDrag() {
        this.isDragging = false;
        this.draggedWaypointIndex = null;
    ...
    /**
     * Get the index of the waypoint being dragged
     * @returns {number|null} Dragged waypoint index or null
     */
    getDraggedIndex() {
        return this.draggedWaypointIndex;
    }
    ...
    getDragState() {
        return {
            ...
            segmentColor: this._dragSegmentColor,
            segmentMode: this._dragSegmentMode,
            isViaInsert: this._viaInsertedByDrag
        };
    }

    /**
     * Get stored segment color for drag preview
    ...
    /**
     * Get stored segment mode for routing
     * @returns {string|null} Segment mode
     */
    getDragSegmentMode() {
        return this._dragSegmentMode;
    }
}
