...
} from '../overpass-network.js';

import { PoiManager } from './managers/PoiManager.js';
import { ElevationProfileManager } from './managers/ElevationProfileManager.js';
import { RouteManager } from './managers/RouteManager.js';
import { WaypointManager } from './managers/WaypointManager.js';
import { BivouacManager } from './managers/BivouacManager.js';
import { StatsManager } from './managers/StatsManager.js';
import { ImportExportManager } from './managers/ImportExportManager.js';
import { RouteDisplayManager } from './managers/RouteDisplayManager.js';
import { MapInteractionManager } from './managers/MapInteractionManager.js';
...
export class DirectionsManager {
  ...
  constructor(map, uiElements = [], options = {}) {
    ...
    // Initialize PoiManager for delegated POI handling
    this.poiManager = new PoiManager({
      map: this.map,
      getRouteProfile: () => this.routeProfile,
      getOfflinePoiCollection: () => this.offlinePoiCollection,
      getCutSegments: () => this.cutSegments,
      getCurrentMode: () => this.currentMode,
      getModeColors: () => this.modeColors,
      getProfileMode: () => this.profileMode,
      onPoisUpdated: (pois) => {
        this.routePointsOfInterest = pois;
        // Re-render route stats to show POIs
    ...
    // Initialize ElevationProfileManager for chart utilities
    this.elevationProfileManager = new ElevationProfileManager({
      elevationChart: this.elevationChart,
      elevationChartBody: this.elevationChartBody,
      getRouteProfile: () => this.routeProfile,
      getCutSegments: () => this.cutSegments,
      getProfileSegments: () => this.profileSegments,
      getCurrentMode: () => this.currentMode,
      getModeColors: () => this.modeColors,
      getRouteGeojson: () => this.routeGeojson,
      callbacks: {
        onHover: (distanceKm) => this.updateElevationHoverReadout(distanceKm),
        onContextMenu: (distanceKm) => this.addRouteCut(distanceKm),
        addRouteCut: (distanceKm) => this.addRouteCut(distanceKm)
      }
    });

    // Initialize RouteManager for route calculation utilities
    this.routeManager = new RouteManager({
      getWaypoints: () => this.waypoints,
      callbacks: {
        onRouteUpdated: (profile) => {
          // Sync route profile if RouteManager computes it
          if (profile) {
    ...
    // Initialize WaypointManager for waypoint state and history
    this.waypointManager = new WaypointManager({
      map: this.map,
      getRouteCutDistances: () => this.routeCutDistances,
      getCachedLegSegments: () => this.cachedLegSegments,
      cloneCachedLegSegments: () => this.cloneCachedLegSegments(),
      restoreCachedLegSegments: (segments) => this.restoreCachedLegSegments(segments),
      callbacks: {
        onWaypointsChanged: () => {
          // Sync waypoints back from manager
          this.waypoints = this.waypointManager.getWaypoints();
          this.updateWaypoints();
        },
        onHistoryChanged: () => {
          this.updateUndoAvailability();
        },
        resolveWaypointColor: (coords, index, total) => this.resolveWaypointColor(coords, index, total),
        cloneRouteCuts: () => this.cloneRouteCuts(),
        setRouteCutDistances: (cuts) => this.setRouteCutDistances(cuts),
        onDragStart: (index, info) => {
          this.map.dragPan?.disable();
          this.map.getCanvas().style.cursor = 'grabbing';
        },
        onDragEnd: (index, result) => {
          this.map.dragPan?.enable();
          this.map.getCanvas().style.cursor = '';
          this.clearDragPreview();
          this.resetDragWaypointColor();
        },
        onDragMove: (index, coords, info) => {
          this.updateWaypoints();
          this.updateDragPreview(index, coords);
          this.updateDragWaypointColor(index);
        }
      }
    });

    // Initialize BivouacManager for bivouac/day segment management
    this.bivouacManager = new BivouacManager({
      map: this.map,
      getRouteProfile: () => this.routeProfile,
      getCoordinateAtDistance: (km) => this.getCoordinateAtDistance(km),
      extractCoordinatesBetween: (startKm, endKm) => this.extractCoordinatesBetween(startKm, endKm),
      callbacks: {
        onCutSegmentsChanged: () => this.updateCutDisplays(),
        recordState: () => this.recordWaypointState(),
        getOrCreateBivouacIcon: (color) => this.getOrCreateBivouacIcon?.(color) || 'segment-marker-bivouac'
      }
    });

    // Initialize StatsManager for route statistics
    this.statsManager = new StatsManager({
      statsPanel: this.routeStats,
      getRouteProfile: () => this.routeProfile,
      getCutSegments: () => this.cutSegments,
      getRoutePointsOfInterest: () => this.routePointsOfInterest,
      callbacks: {
        onDaySelected: (dayIndex) => this.zoomElevationChartToDay(dayIndex)
      }
    });

    // Initialize ImportExportManager for route import/export
    this.importExportManager = new ImportExportManager({
      getRouteGeojson: () => this.routeGeojson,
      getCutSegments: () => this.cutSegments,
      getWaypoints: () => this.waypoints,
      getRouteProfile: () => this.routeProfile,
      computeSegmentMarkers: () => this.computeSegmentMarkers(),
      callbacks: {
        onImport: (data) => this.applyImportedRoute(data)
      }
    });

    // Initialize RouteDisplayManager for route line rendering
    this.routeDisplayManager = new RouteDisplayManager({
      map: this.map,
      getRouteGeojson: () => this.routeGeojson,
      getCutSegments: () => this.cutSegments,
      getProfileSegments: () => this.profileSegments,
      ...
      getModeColors: () => this.modeColors,
      callbacks: {
        computeRouteOverlapOffsets: (coords) => this.computeRouteOverlapOffsets?.(coords) || { offsets: [], isOverlap: [] },
        geometricOffsetCoordinates: (coords, offsets, scale) => this.geometricOffsetCoordinates?.(coords, offsets, scale) || coords
      }
    });

    // Initialize MapInteractionManager for event delegation
  ...
  setupMapHandlers() {
    // Delegate to MapInteractionManager for centralized event handling
    if (this.mapInteractionManager) {
      this.mapInteractionManager.setupEventHandlers();
    }
  }
  ...
  setRoutePointsOfInterest(pois) {
    // Delegate to PoiManager if available
    if (this.poiManager) {
      this.poiManager.setRoutePointsOfInterest(pois);
      return;
    }
  ...
  updateRoutePoiData() {
    // Delegate to PoiManager if available
    if (this.poiManager) {
      this.poiManager.updateRoutePoiData();
      return;
    }
  ...
  updateRoutePoiLayerVisibility() {
    // Delegate to PoiManager if available
    if (this.poiManager) {
      this.poiManager.updateRoutePoiLayerVisibility();
      return;
    }
  ...
  updatePoiDayColors() {
    // Delegate to PoiManager if available
    if (this.poiManager) {
      this.poiManager.updatePoiDayColors();
      return;
    }
  ...
  onWaypointMouseDown(event) {
    ...
    const waypointIndex = Number(feature.properties.index);
    if (!Number.isFinite(waypointIndex)) return;

    // Calculate segment color based on position
    let segmentColor = this.modeColors[this.currentMode];
    if (this.routeProfile && Number.isFinite(this.routeProfile.totalDistanceKm) && this.waypoints.length > 1) {
      ...
      segmentColor = this.getColorForDistance(estimatedDistanceKm) || segmentColor;
    }

    // Get routing mode from adjacent segments
    ...
    const segmentMode = prevMode || nextMode || this.currentMode;

    // Use WaypointManager to start drag (callbacks handle cursor, dragPan)
    this.waypointManager.startDrag(waypointIndex, {
      prevNeighbor: this.waypoints[waypointIndex - 1]?.slice(0, 2) ?? null,
      nextNeighbor: this.waypoints[waypointIndex + 1]?.slice(0, 2) ?? null,
      segmentColor,
      segmentMode
    });

    // Sync local state (needed while transitioning to full delegation)
    this.isDragging = true;
    this.draggedWaypointIndex = waypointIndex;
    this._dragPrevNeighbor = this.waypointManager._dragPrevNeighbor;
    this._dragNextNeighbor = this.waypointManager._dragNextNeighbor;
    this._dragSegmentColor = segmentColor;
    this._dragSegmentMode = segmentMode;
  ...
  onMapMouseDown(event) {
    ...
    // Immediately start dragging the inserted waypoint via WaypointManager
    this.waypointManager.startDrag(insertIndex, {
      prevNeighbor: prevNeighborCoords,
      nextNeighbor: nextNeighborCoords,
      segmentColor: capturedSegmentColor,
      segmentMode: capturedSegmentMode,
      isViaInsert: true
    });

    // Sync local state (needed during transition)
    this.isDragging = true;
    this.draggedWaypointIndex = insertIndex;
    this._viaInsertedByDrag = true;
    this._dragPrevNeighbor = prevNeighborCoords;
    this._dragNextNeighbor = nextNeighborCoords;
  ...
  onMapMouseMove(event) {
    ...
    // Handle waypoint dragging via WaypointManager
    if (this.waypointManager.getIsDragging()) {
      const coords = [event.lngLat.lng, event.lngLat.lat];
      const builtCoords = this.buildWaypointCoordinate(coords) ?? coords;

      // Update waypoint position via WaypointManager (callback handles preview)
      this.waypointManager.updateDragPosition(builtCoords);

      // Also update local state for backward compatibility
      if (this.draggedWaypointIndex !== null) {
        this.waypoints[this.draggedWaypointIndex] = builtCoords;
      }
    }
  ...
  onMapMouseUp(event) {
    ...
    // Finish waypoint drag via WaypointManager (callbacks handle cursor, cleanup)
    if (this.waypointManager.getIsDragging()) {
      this.waypointManager.finishDrag();
    }
    ...
    // Always ensure cleanup happens (fallback if callbacks don't fire)
    this.map.dragPan?.enable();
    this.map.getCanvas().style.cursor = '';
    this.clearDragPreview();
    this.resetDragWaypointColor();
  ...
  */
  ...
  computeDistanceKm(startCoord, endCoord) {
    ...
    const dLat = (endCoord[1] - startCoord[1]) * Math.PI / 180;
  ...
  buildRouteProfile(coordinates = []) {
    ...
    return this.routeManager.buildRouteProfile(coordinates, {
      elevationProvider: canQueryTerrain
        ? (coord) => this.queryTerrainElevationValue(coord)
        : null
    });
  }
  ...
  calculateRouteMetrics(route) {
    const metrics = this.routeManager.calculateRouteMetrics(route);
    return {
      distanceKm: metrics.distanceKm,
      ascent: metrics.ascentMeters,
      descent: metrics.descentMeters
    };
  }
...
`;
  ...
  async refreshRoutePointsOfInterest() {
    // Delegate to PoiManager if available
    if (this.poiManager) {
      await this.poiManager.refreshRoutePointsOfInterest();
      // Update elevation profile after POI refresh
      const coords = this.routeGeojson?.geometry?.coordinates;
      if (Array.isArray(coords) && coords.length >= 2) {
