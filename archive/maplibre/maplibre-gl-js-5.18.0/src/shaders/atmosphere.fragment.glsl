...
void main() {
    ...
    // Add sun disk and glare
    vec3 v = normalize(view_direction);
    vec3 s = normalize(u_sun_pos);
    float mu = dot(v, s);
    
    // Sun disk (~0.25 deg radius)
    float sun_disk = smoothstep(0.99998, 0.99999, mu);
    
    // Tighter Sun glare
    float sun_glare = pow(max(0.0, mu), 2048.0) * 0.4;
    sun_glare += pow(max(0.0, mu), 128.0) * 0.1;
    
    float horizon_mask = smoothstep(-0.01, 0.05, s.y); 
    
    vec3 sun_color = vec3(1.0, 0.98, 0.95) * 100.0 * sun_disk;
    sun_color += vec3(1.0, 0.9, 0.8) * 2.0 * sun_glare;
    
    // Moon rendering
    vec3 moon_dir = normalize(u_moon_pos);
    float mu_moon = dot(v, moon_dir);
    float moon_fmt = smoothstep(0.99998, 0.99999, mu_moon);
    vec3 moon_color = vec3(0.5, 0.5, 0.5) * 40.0 * moon_fmt;
    
    color.rgb += (sun_color + moon_color) * color.a;
