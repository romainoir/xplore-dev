...
import {isShadowStyleLayer, type ShadowStyleLayer} from '../style/style_layer/shadow_style_layer';
import {isDaylightStyleLayer} from '../style/style_layer/daylight_style_layer';
...
function atlasBoundsContain(atlasBounds: number[], requestedBounds: MercatorBounds): boolean {
    const epsilon = 1e-12;
...
export class Painter {
    ...
    /**
     * Update terrain depth/coords and Xplore's world-space elevation/shadow atlases.
     * Atlas refreshes are skipped while the camera is moving so the last stable shadow
     * stays on screen without blocking pan/rotate/zoom.
     */
    maybeDrawDepthAndCoords(requireExact: boolean) {
        ...
        const tilesChanged = terrain.tileManager.anyTilesAfterTime(this.terrainFacilitator.renderTime);
        ...
        let doUpdate = this.terrainFacilitator.depthDirty || this.terrainFacilitator.coordsDirty;
        ...
        this.drawFunctions.terrainCoords(this, terrain);
    ...
    renderLayer(painter: Painter, tileManager: TileManager, layer: StyleLayer, coords: OverscaledTileID[], renderOptions: RenderOptions) {
        ...
        if (isSymbolStyleLayer(layer)) {
            ...
            draw.shadow(painter, tileManager, layer, coords, renderOptions);
        } else if (isDaylightStyleLayer(layer)) {
            draw.daylight(painter, tileManager, layer, coords, renderOptions);
