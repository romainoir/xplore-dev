...
export class Style extends Evented {
    ...
    addLayer(layerObject: AddLayerObject, before?: string, options: StyleSetterOptions = {}): this {
        ...
        if (layerObject.type === 'custom') {
            ...
            // Xplore renderer-owned layers are not part of the public style spec yet.
            ...
            layer = createStyleLayer(layerObject as any, this._globalState);
    ...
    serialize(): StyleSpecification | undefined {
        ...
        const serializableOrder = this._order.filter(id => {
            const type = (this._layers[id] as any)?.type;
            return type !== 'shadow' && type !== 'daylight';
        });
        const layers = this._serializeByIds(serializableOrder, true);
