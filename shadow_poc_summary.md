...
- **Non-blurred shadow contacts**: Cast shadows no longer run the atlas blur pass by default. The global raymarch keeps the softer contact response, while the terrain shader applies a tiny jittered edge-only atlas filter and snaps high-confidence shadow interiors to a solid mask. This keeps the preferred soft transition look while hiding broad internal quantization bands without mask dithering.
